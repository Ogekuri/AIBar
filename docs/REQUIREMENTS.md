...
- **REQ-017**: Provider cards with quota fields MUST render `Remaining credits: <remaining> / <limit>` with `<remaining>` in bold bright white; reset labels in extension cards MUST start with `Reset in:` and append `⚠️ Limit reached!` at displayed `100.0%`; failed refresh states MUST render explicit error diagnostics and MUST hide usage/reset/quota/cost metric rows; Copilot/OpenRouter/OpenAI/GeminiAI cards MUST render a single `30d` window bar with reset text directly below that bar (and before remaining-credits text when present); popup header labels MUST render `AIBar` branding and MUST NOT render `Open AIBar Report`; each provider card tab MUST render `Updated: <datetime>, Next: <datetime>` aligned bottom-right inside each provider card container using `aibar-reset-label` font/color, where `Updated` is sourced from `freshness.<provider>.last_success_timestamp`, `Next` is sourced from `freshness.<provider>.idle_until_timestamp`, and datetime format MUST use local timezone in `%Y-%m-%d %H:%M` format and match CLI `show` output formatting; provider-card fallback freshness MUST derive `Next` as `status.<provider>.<window>.updated_at + extension.idle_delay_seconds`; provider-card monetary rows for `openai`, `openrouter`, `codex`, and `geminiai` MUST render `Cost: <currency><value>` with `<currency><value>` in bold bright white; for providers exposing API counters (`openai`, `openrouter`, `codex`, `geminiai`), labels on `OK` states MUST render `Requests: <requests>` and `Tokens: <total> (<in> in / <out> out)` with null counters normalized to `0`.
...
- **REQ-036**: CLI text `show` MUST suppress statistics lines (`Usage`, percentage bars, `Resets in`, `Remaining credits`, request/token/cost values) when provider/window status is `FAIL`, rendering only freshness and failure diagnostics; for providers exposing API counters (`openai`, `openrouter`, `codex`, `geminiai`), lines on `OK` states MUST render `Requests: <requests>` and `Tokens: <total> (<in> in / <out> out)` with null counters normalized to `0`.
...
- **REQ-060**: CLI text `show` and JSON `show --json` MUST expose GeminiAI monitoring usage fields, latest-available billing-period monetary cost values, and cached `result/error` status for billing table discovery or query failures; CLI text MUST insert one blank line before `Billing table`.
...
- **REQ-067**: CLI `show` text mode MUST render ANSI provider-colored bordered panels ordered `claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`; each panel MUST render `Status` first and `Updated/Next` last, both separated by one blank line.
...
- **TST-004**: MUST verify GNOME extension error path sets panel text `Err`, caps displayed error string length to 40 characters, hides usage/reset/quota/cost metric rows for failed refresh states, renders explicit error diagnostics (including retry-after metadata when available), renders Copilot/OpenRouter/OpenAI/GeminiAI `30d` bar/reset placement on successful states, renders popup header label `AIBar` and omits `Open AIBar Report`, verifies popup `Refresh Now` executes with `--force`, verifies provider-card area uses scroll containment for overflowed cards, verifies each provider card renders bottom-right `Updated: <datetime>, Next: <datetime>` from `freshness.<provider>.last_success_timestamp` and `freshness.<provider>.idle_until_timestamp` with local-timezone `%Y-%m-%d %H:%M` formatting identical to CLI `show`, verifies fallback freshness computes `Next` as `status.<provider>.<window>.updated_at + extension.idle_delay_seconds`, verifies `openai/openrouter/codex/geminiai` cost rows render `Cost: <currency><value>` with bold bright-white value, and verifies API-counter providers (`openai`, `openrouter`, `codex`, `geminiai`) render `Requests: <requests>` and `Tokens: <total> (<in> in / <out> out)` with null counters normalized to `0`.
...
- **TST-030**: MUST verify CLI `show` renders panels ordered `claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`, renders `Status` first and `Updated/Next` last with blank-line separators, and keeps one shared panel width across panels.
...
- **TST-038**: MUST verify CLI text `show` renders `Updated: <datetime last update>, Next: <datetime next update>` from provider `idle_time` timestamps with local-timezone `%Y-%m-%d %H:%M` formatting aligned to GNOME extension, verifies `show --json` exports equivalent provider freshness in top-level `freshness`, suppresses statistics lines on `FAIL` states, renders `HTTP status: <code>, Retry after: <seconds> sec.` when available, renders provider API counters (`openai`, `openrouter`, `codex`, `geminiai`) as `Requests: <requests>` and `Tokens: <total> (<in> in / <out> out)` with null values normalized to `0` on `OK` states, renders cost lines as `Cost: <currency><value>` with bold bright-white value, renders GeminiAI effective window as `30d` regardless of requested window, and inserts one blank line before GeminiAI `Billing table`.
...
| REQ-003 | `src/aibar/aibar/cli.py` + `show` JSON renderer emits `indent=2` with `payload`, `status`, `idle_time`, `freshness`, and `extension` sections; `freshness` exports provider-keyed timestamps/local-time strings and `extension` exports `gnome_refresh_interval_seconds` plus `idle_delay_seconds`. |
...
| REQ-017 | `src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_buildPopupMenu/_parseOutput/_updateUI/_populateProviderCard` + popup removes `Open AIBar Report`, `copilot/openrouter/openai/geminiai` render single `30d` window bars, freshness uses `freshness` data with status+`extension.idle_delay_seconds` fallback, and API-counter/cost/reset labels remain normalized. |
...
| TST-004 | `tests/test_extension_quota_label.py` + popup-header/remove-action assertions, failed-state metric-row suppression, 30d-bar assertions for `copilot/openrouter/openai/geminiai`, freshness fallback assertions via `extension.idle_delay_seconds`, bottom-right `Updated/Next` assertions, and API-counter format assertions for `openai/openrouter/codex/geminiai`. |
...
| TST-030 | `tests/test_cli_show_panel_alignment.py` + CLI output assertions verify provider order, grouped dual-window rendering, `Status` first/`Updated/Next` last blank-line layout, and identical visible panel width. |
| REQ-067 | `src/aibar/aibar/cli.py` + `show/_provider_panel_sort_key/_build_result_panel/_build_dual_window_panel/_emit_provider_panel` + provider ordering, status-first and freshness-last row placement, ANSI colors, and shared-width rendering. |
...
| TST-038 | `tests/test_cli_show_status_messages.py` + verifies CLI `show` freshness line uses provider `idle_time` timestamps with local-timezone `%Y-%m-%d %H:%M` parity, `show --json` top-level `freshness` parity, fail-state statistics suppression, retry-after rendering format, API-counter null→`0` rendering for `openai/openrouter/codex/geminiai`, and GeminiAI effective window `30d` under non-30d requests. |
