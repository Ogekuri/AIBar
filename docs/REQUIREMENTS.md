...
| REQ-017 | `src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_buildPopupMenu/_parseOutput/_updateUI/_populateProviderCard` + provider-card scroll containment preserves bottom-right freshness label visibility; `openai/openrouter/codex/geminiai` labels render `<requests> requests` and `<total> tokens (<in> in / <out> out)` with null→`0`. |
...
| TST-004 | `tests/test_extension_quota_label.py` + popup-label assertions, failed-state metric-row suppression, explicit error+retry metadata, provider-scroll containment, bottom-right `Updated/Next` assertions, and API-counter format assertions `<requests> requests` + `<total> tokens (<in> in / <out> out)` for `openai/openrouter/codex/geminiai`. |
