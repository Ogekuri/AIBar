...
- **REQ-005**: MUST prompt `setup` for `idle_delay_seconds` (default `300`) first, `api_call_delay_milliseconds` (default `100`) second, `api_call_timeout_milliseconds` (default `5000`) third, `gnome_refresh_interval_seconds` (default `60`) fourth, and `billing_data` (default `billing_data`) fifth, then persist values in `~/.config/aibar/config.json`.
...
- **REQ-021**: MUST render GNOME panel status labels after the icon in this order: Claude 5h percentage, Claude 7d percentage, Claude cost, OpenRouter cost, Copilot 30d percentage, Codex 5h percentage, Codex 7d percentage, Codex cost, OpenAI cost, GeminiAI cost; window percentage labels with `status.result=FAIL` MUST render `Err`, and provider cost labels MUST NOT render `Err`.
...
- **TST-007**: MUST verify GNOME panel status labels render in the REQ-021 order (including OpenAI cost), map `Err` only to failed percentage labels, never map `Err` to cost labels, enforce provider style classes and color mapping, verify icon color/blink thresholds, render zero-cost currency labels, and omit unavailable metrics.
...
| REQ-005 | `src/aibar/aibar/cli.py` + `setup` prompts `idle_delay_seconds`, `api_call_delay_milliseconds`, `api_call_timeout_milliseconds`, and `gnome_refresh_interval_seconds` with defaults `300`, `100`, `5000`, and `60`, then persists `~/.config/aibar/config.json`. |
...
| REQ-021 | `src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_buildPanelButton/_updateUI` + panel labels inserted between icon and summary label in fixed order: Claude 5h, Claude 7d, Claude cost, OpenRouter cost, Copilot 30d, Codex 5h, Codex 7d, Codex cost, OpenAI cost, GeminiAI cost; `Err` projection applies only to failed percentage labels. |
...
| TST-007 | `tests/test_extension_quota_label.py` + panel-segment order assertions (including OpenAI cost), failed-percentage `Err` projection assertions, cost-label non-`Err` assertions, provider style classes, zero-cost currency-label visibility checks, and missing-metric omission behavior. |
