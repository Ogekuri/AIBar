...
| TST-004 | `tests/test_extension_quota_label.py` + popup-label assertions (`AIBar`, `Open AIBar Report`), failed-state metric-row suppression assertions, explicit error+retry metadata assertions, bottom-left `Updated/Next` provider-card label assertions sourced from `freshness` timestamps with local-timezone `%Y-%m-%d %H:%M`, and API-counter null→`0` rendering assertions for `openai/openrouter/codex/geminiai`; `tests/test_extension_dev_script.py` + `MUTTER_DEBUG_DUMMY_MODE_SPECS=1024x800` and no `aibar gnome-install` invocation. |
