...
- **DES-006**: MUST auto-refresh GNOME extension data every configured scheduler interval, support manual refresh from the popup menu, and render status text as `Last updated: <time>, next update: <time>` based on that scheduler.
...
| DES-006 | `src/aibar/gnome-extension/aibar@aibar.panel/extension.js` + scheduler-driven `_startAutoRefresh/_refreshData` plus status labels rendering `Last updated: <time>, next update: <time>`. |
...
| TST-004 | `tests/test_extension_quota_label.py` + popup-label assertions (`AIBar`, `Open AIBar UI`), reset-prefix and `⚠️ Limit reached!` assertions, rate-limit error-string suppression assertions, and `Last updated`/`next update` status-line assertions; `tests/test_extension_dev_script.py` + `MUTTER_DEBUG_DUMMY_MODE_SPECS=1024x800`; `src/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_handleError/_populateProviderCard/_buildPopupMenu` coverage. |
