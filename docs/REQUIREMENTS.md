...
- **REQ-002**: MUST render default-window Claude and Codex as one panel per provider with blank-line-separated `5h` and `7d` sections; Codex `Cost/Requests/Tokens` lines MUST appear after `7d`, separated by one blank line.
...
- **REQ-036**: CLI text `show` MUST suppress statistics lines (`Usage`, percentage bars, `Resets in`, `Remaining credits`, request/token/cost values) when provider/window status is `FAIL`, rendering only freshness and failure diagnostics; for providers exposing API counters (`openai`, `openrouter`, `codex`, `geminiai`), `Requests` and `Tokens` lines MUST render on `OK` states with null counter values normalized to `0`.
...
- **REQ-060**: CLI text `show` and JSON `show --json` MUST expose GeminiAI monitoring usage fields, latest-available billing-period monetary cost values, and cached `result/error` status for billing table discovery or query failures.
