...
| REQ-017 | `src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_parseOutput/_updateUI/_populateProviderCard` + freshness label renders bottom-right inside provider card container using `aibar-reset-label` style and `freshness.<provider>.last_success_timestamp`/`freshness.<provider>.idle_until_timestamp` formatted in local timezone `%Y-%m-%d %H:%M`; `openai/openrouter/codex/geminiai` `requests`/`tokens` labels render with null→`0` normalization on `OK` states. |
...
| TST-004 | `tests/test_extension_quota_label.py` + popup-label assertions (`AIBar`, `Open AIBar Report`), failed-state metric-row suppression assertions, explicit error+retry metadata assertions, bottom-right `Updated/Next` provider-card label assertions inside provider-card container sourced from `freshness` timestamps with local-timezone `%Y-%m-%d %H:%M`, and API-counter null→`0` rendering assertions for `openai/openrouter/codex/geminiai`; `tests/test_extension_dev_script.py` + `MUTTER_DEBUG_DUMMY_MODE_SPECS=1024x800` and no `aibar gnome-install` invocation. |
