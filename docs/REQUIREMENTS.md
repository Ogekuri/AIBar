...
- **REQ-102**: GNOME extension popup window MUST enforce a fixed standard width and a fixed height, word-wrap verbose error messages, and constrain the maximum width of tab content and status bars to prevent UI expansion.
...
- **TST-052**: MUST verify GNOME extension stylesheet applies fixed width and height to the popup menu, wraps verbose error message labels, and constrains tab content and status bar lengths to prevent UI layout expansion.
