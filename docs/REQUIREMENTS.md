...
- **REQ-128**: CLI `show` text mode MUST render `Usage: <window> <progress_bar> <percent>%` for `claude`, `openrouter`, `copilot`, and `codex`.
...
| REQ-122 | `src/aibar/aibar/cli.py` + `_build_cli_usage_line/_build_result_panel/_progress_bar` + >100 progress-bar rendering is limited to `claude/openrouter/copilot/codex` while preserving marker and over-limit segment. |
| REQ-128 | `src/aibar/aibar/cli.py` + `_build_cli_usage_line/_build_result_panel` + usage rows render `Usage: <window> <progress_bar> <percent>%` for bar-enabled providers only. |
| REQ-131 | `src/aibar/aibar/cli.py` + `_build_cli_usage_line/_build_result_panel` + `openai/geminiai` usage rows render `Usage: <window> <percent>%` without progress bars. |
...
| TST-054 | `tests/test_cli_show_panel_alignment.py` + output assertions verify bar providers keep `Usage: <window> <progress_bar> <percent>%` with visible `|`/`▓`, while `openai/geminiai` render `Usage: <window> <percent>%` and preserve shared panel width. |
