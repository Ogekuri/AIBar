...
- **REQ-116**: `setup` MUST present a dedicated Copilot premium section after `billing_data` that prompts `copilot_extra_premium_request_cost` with default `0.004` USD per premium request and persists it in `~/.config/aibar/config.json`.
...
- **REQ-067**: CLI `show` text mode MUST render ANSI provider-colored bordered panels ordered `claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`, MUST render `Status` first on `OK`, MUST include Copilot `Extra premium cost: <currency_symbol><value>`, and MUST NOT render `Window <window>` heading text.
- **REQ-117**: GNOME Copilot card MUST render `Extra premium cost: <currency_symbol><value>` from `premium_requests_extra_cost`, including `0`, preserving existing Copilot typography and spacing.
- **REQ-118**: GNOME panel status bar MUST place Copilot `+<currency_symbol><extra_cost>` immediately after Copilot percentages and before Codex status labels when Copilot extra premium cost is available.
...
- **TST-013**: MUST verify `setup` prompts idle-delay first, API-call delay milliseconds second (default `100`), API-call timeout milliseconds third (default `5000`), `gnome_refresh_interval_seconds` fourth, `billing_data` fifth, Copilot extra-premium cost sixth (default `0.004`), then currency prompts, then final logging prompts, and persists all values into `~/.config/aibar/config.json`.
...
- **TST-030**: MUST verify CLI `show` renders panels ordered `claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`, keeps one shared panel width, renders Copilot `Extra premium cost: <currency_symbol><value>`, never renders `Window 5h:/7d:/30d:` headings, and renders `Status` then right-aligned `Updated/Next` for both `OK` and `FAIL` states.
- **TST-053**: MUST verify GNOME Copilot card renders `Extra premium cost: <currency_symbol><value>` and panel status bar renders Copilot `+<currency_symbol><extra_cost>` between Copilot percentages and Codex labels.
