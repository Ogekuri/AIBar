...
- **REQ-067**: CLI `show` text mode MUST render ANSI provider-colored bordered panels ordered `claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`; each panel MUST render `Status` first, MUST render window headings as `Window <window>:`, and MUST render the final `Updated/Next` line right-aligned to panel content width.
...
- **TST-004**: MUST verify GNOME extension error path sets panel text `Err`, caps displayed error string length to 40 characters, hides usage/reset/quota/cost metric rows for failed refresh states, renders explicit error diagnostics (including retry-after metadata when available), renders Copilot/OpenRouter/OpenAI/GeminiAI `30d` bar/reset placement with visible `30d` left label on successful states, renders popup header label `AIBar` and omits `Open AIBar Report`, verifies popup `Refresh Now` executes with `--force`, verifies provider-card area uses scroll containment for overflowed cards, verifies each provider card renders bottom-right `Updated: <datetime>, Next: <datetime>` from `freshness.<provider>.last_success_timestamp` and `freshness.<provider>.idle_until_timestamp` with local-timezone `%Y-%m-%d %H:%M` formatting identical to CLI `show`, verifies fallback freshness computes `Next` as `status.<provider>.<window>.updated_at + extension.idle_delay_seconds`, verifies all provider cost rows render `Costs: <currency><value>` with label/value style parity and no blank spacer row after `Costs:` on OpenRouter/OpenAI/GeminiAI cards, and verifies API-counter providers (`openai`, `openrouter`, `codex`, `geminiai`) render `Requests: <requests>` and `Tokens: <total> (<in> in / <out> out)` with null counters normalized to `0`.
...
- **TST-030**: MUST verify CLI `show` renders panels ordered `claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`, renders `Status` first, renders window headings as `Window <window>:`, renders right-aligned `Updated/Next` as last row, and keeps one shared panel width across panels.
...
| REQ-017 | `src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_buildPopupMenu/_parseOutput/_updateUI/_populateProviderCard` + popup removes `Open AIBar Report`, `copilot/openrouter/openai/geminiai` render single `30d` window bars with left `30d` labels, freshness uses `freshness` data with status+`extension.idle_delay_seconds` fallback, `Costs:` rows use `Remaining credits` style parity without bold value override, and OpenRouter/OpenAI/GeminiAI cards suppress empty spacer rows after `Costs:` while API counters stay normalized. |
...
| TST-004 | `tests/test_extension_quota_label.py` + popup-header/remove-action assertions, failed-state metric-row suppression, 30d-bar left-label assertions for `copilot/openrouter/openai/geminiai`, freshness fallback assertions via `extension.idle_delay_seconds`, bottom-right `Updated/Next` assertions, `Costs:` style/no-spacer assertions, and API-counter format assertions for `openai/openrouter/codex/geminiai`. |
