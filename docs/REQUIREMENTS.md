...
- **REQ-021**: MUST render GNOME panel status labels after the icon in this order: Claude 5h percentage, Claude 7d percentage, Claude cost, OpenRouter cost, Copilot 30d percentage, Codex 5h percentage, Codex 7d percentage, Codex cost, OpenAI cost, GeminiAI cost; percentage labels for `status.result=FAIL` provider/windows MUST render `Err`.
