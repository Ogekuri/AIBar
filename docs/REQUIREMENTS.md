...
- **TST-004**: MUST verify GNOME extension error path sets panel text `Err`, caps displayed error string length to 40 characters, hides usage/reset/quota/cost metric rows for failed refresh states, renders explicit error diagnostics (including retry-after metadata when available), renders Copilot/OpenRouter/OpenAI/GeminiAI `30d` bar/reset placement with left label sourced from `show --json` `extension.window_labels.<provider>` (or static fallback `30d`) on successful states, renders popup header label `AIBar` and omits `Open AIBar Report`, verifies popup `Refresh Now` executes with `--force`, verifies provider-card area uses scroll containment for overflowed cards, verifies each provider card renders bottom-right `Updated: <datetime>, Next: <datetime>` from `freshness.<provider>.last_success_timestamp` and `freshness.<provider>.idle_until_timestamp` with local-timezone `%Y-%m-%d %H:%M` formatting identical to CLI `show`, verifies fallback freshness computes `Next` as `status.<provider>.<window>.updated_at + extension.idle_delay_seconds`, verifies all provider cost rows render `Costs: <currency><value>` with numeric values (including currency symbol) in bold bright white and no blank spacer row after `Costs:` on OpenRouter/OpenAI/GeminiAI cards, and verifies API-counter providers (`openai`, `openrouter`, `codex`, `geminiai`) render `Requests: <requests>` and `Tokens: <total> (<in> in / <out> out)` with null counters normalized to `0`.
