...
- **REQ-084**: CLI text `show` MUST render per-provider freshness line as `Updated: <datetime last update>, Next: <datetime next update>` where datetime MUST use local timezone in `%Y-%m-%d %H:%M` format and use the same formatting rule as GNOME extension provider cards.
...
- **TST-042**: MUST verify CLI `show` and GNOME provider cards render equivalent error category, HTTP status, retry-after value, and `Updated/Next` freshness for the same failed provider/window cache status.
...
| REQ-017 | `src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_updateUI/_populateProviderCard/_buildPopupMenu` + successful-state quota/reset/card rendering unchanged; failed-state branch hides usage/reset/quota/cost metric rows while rendering explicit error diagnostics; `Updated: <datetime>, Next: <datetime>` label remains local-timezone `%Y-%m-%d %H:%M` aligned with CLI `show`. |
...
| TST-004 | `tests/test_extension_quota_label.py` + popup-label assertions (`AIBar`, `Open AIBar Report`), failed-state metric-row suppression assertions, explicit error+retry metadata assertions, and `Updated: <datetime>, Next: <datetime>` provider-card label assertions with local-timezone `%Y-%m-%d %H:%M` formatting parity against CLI `show`; `tests/test_extension_dev_script.py` + `MUTTER_DEBUG_DUMMY_MODE_SPECS=1024x800`; `src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_handleError/_populateProviderCard/_buildPopupMenu` coverage. |
...
| REQ-084 | `src/aibar/aibar/cli.py` + `_print_result` + per-provider freshness line renders `Updated: <datetime>, Next: <datetime>` with local-timezone `%Y-%m-%d %H:%M` formatting aligned to extension cards. |
...
| TST-038 | `tests/test_cli_show_status_messages.py` + verifies CLI `show` freshness line local-timezone `%Y-%m-%d %H:%M` parity, fail-state statistics suppression, and retry-after rendering format. |
| TST-042 | `tests/test_cli_show_status_messages.py` + `tests/test_extension_quota_label.py` + cross-surface assertions for equivalent error category, HTTP status, retry-after value, and `Updated/Next` text on failed states. |
