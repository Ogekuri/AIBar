...
- **REQ-067**: CLI `show` text mode MUST render ANSI provider-colored bordered panels ordered `claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`, MUST render `Status` first on `OK`, MUST include Copilot `Cost: <currency_symbol><value>`, and MUST NOT render `Window <window>` heading text.
...
- **TST-030**: MUST verify CLI `show` renders panels ordered `claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`, keeps one shared panel width, renders Copilot `Cost: <currency_symbol><value>`, never renders `Window 5h:/7d:/30d:` headings, and renders `Status` then right-aligned `Updated/Next` for both `OK` and `FAIL` states.
