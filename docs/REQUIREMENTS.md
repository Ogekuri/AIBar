...
- **REQ-128**: CLI `show` text mode MUST render each usage row as `Usage: <window> <progress_bar> <percent>%`.
...
- **TST-058**: MUST verify CLI `show` renders Copilot `Cost: <currency_symbol><value>` after one blank line following `Remaining credits`.
...
- **TST-054**: MUST verify CLI text `show` renders >100 usage rows as `Usage: <window> <progress_bar> <percent>%` with visible over-limit segment, preserved 100% boundary marker, and unchanged shared panel width.
...
| REQ-128 | `src/aibar/aibar/cli.py` + `_build_result_panel` + usage rows render `Usage: <window> <progress_bar> <percent>%` using explicit window labels adjacent to the progress bar. |
...
| TST-054 | `tests/test_cli_show_panel_alignment.py` + over-limit CLI usage-row assertions verify `Usage: <window> <progress_bar> <percent>%` ordering, visible `|` marker, visible `▓` segment, and unchanged visible panel width. |
