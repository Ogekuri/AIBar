...
- **DES-006**: MUST auto-refresh GNOME extension data every configured scheduler interval, execute popup `Refresh Now` actions with `aibar show --json --force`, and render status text as `Last updated: <time>, next update: <time>` based on that scheduler.
...
- **REQ-003**: MUST emit pretty-printed JSON (`indent=2`) for `show --json` using the `cache.json` schema that includes per-provider/window payload and last-attempt status sections.
...
| DES-006 | `src/aibar/gnome-extension/aibar@aibar.panel/extension.js` + scheduler-driven `_startAutoRefresh/_refreshData`, popup `Refresh Now` command uses `aibar show --json --force`, and status labels render `Last updated: <time>, next update: <time>`. |
...
| REQ-003 | `src/aibar/aibar/cli.py` + `show` JSON renderer emits `indent=2` and serializes cache-backed payload/status sections from `~/.cache/aibar/cache.json`. |
...
| TST-004 | `tests/test_extension_quota_label.py` + popup-label assertions (`AIBar`, `Open AIBar UI`), reset-prefix and `⚠️ Limit reached!` assertions, rate-limit error-string suppression assertions, `Last updated`/`next update` status-line assertions, and popup `Refresh Now` `--force` assertion; `tests/test_extension_dev_script.py` + `MUTTER_DEBUG_DUMMY_MODE_SPECS=1024x800`; `src/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_handleError/_populateProviderCard/_buildPopupMenu` coverage. |
