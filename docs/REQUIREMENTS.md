...
- **REQ-003**: MUST emit pretty-printed JSON (`indent=2`) for `show --json` with top-level sections `payload`, `status`, `idle_time`, and `extension`, where `idle_time` mirrors provider-scoped CTN-009 entries.
...
- **REQ-017**: Quota-only cards MUST render `Remaining credits: <remaining>/<limit>` with `<remaining>` in bold; reset labels in extension cards MUST start with `Reset in:` and append `⚠️ Limit reached!` at displayed `100.0%`; failed refresh states MUST render explicit error diagnostics and MUST hide usage/reset/quota/cost metric rows; Copilot cards MUST render a `30d` window bar with reset text directly below that bar and before remaining-credits text; popup header/action labels MUST render `AIBar` branding (`AIBar`, `Open AIBar Report`); each provider card tab MUST render `Updated: <datetime>, Next: <datetime>` aligned bottom-right using `aibar-reset-label` font/color, where `Updated` is sourced from `idle_time.<provider>.last_success_timestamp`, `Next` is sourced from `idle_time.<provider>.idle_until_timestamp`, and datetime format MUST use local timezone in `%Y-%m-%d %H:%M` format and match CLI `show` output formatting; for providers exposing API counters (`openai`, `openrouter`, `codex`, `geminiai`), requests/token labels MUST render on `OK` states with null counter values normalized to `0`.
...
- **REQ-084**: CLI text `show` MUST render per-provider freshness line as `Updated: <datetime last update>, Next: <datetime next update>` sourced from provider `idle_time` `last_success_timestamp` and `idle_until_timestamp`, formatted in local timezone `%Y-%m-%d %H:%M` and aligned with GNOME cards.
...
| REQ-003 | `src/aibar/aibar/cli.py` + `show` JSON renderer emits `indent=2` with `payload`, `status`, `idle_time`, and `extension` sections. |
...
| REQ-017 | `src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_parseOutput/_updateUI/_populateProviderCard` + freshness label uses `idle_time.<provider>.last_success_timestamp` and `idle_time.<provider>.idle_until_timestamp` formatted in local timezone `%Y-%m-%d %H:%M`; `openai/openrouter/codex/geminiai` `requests`/`tokens` labels render with null→`0` normalization on `OK` states. |
...
| TST-004 | `tests/test_extension_quota_label.py` + popup-label assertions (`AIBar`, `Open AIBar Report`), failed-state metric-row suppression assertions, explicit error+retry metadata assertions, `Updated/Next` provider-card label assertions sourced from `idle_time` timestamps with local-timezone `%Y-%m-%d %H:%M`, and API-counter null→`0` rendering assertions for `openai/openrouter/codex/geminiai`; `tests/test_extension_dev_script.py` + `MUTTER_DEBUG_DUMMY_MODE_SPECS=1024x800` and no `aibar gnome-install` invocation. |
...
| REQ-084 | `src/aibar/aibar/cli.py` + `_build_result_panel/show` + per-provider freshness line renders `Updated: <datetime>, Next: <datetime>` from provider `idle_time` timestamps with local-timezone `%Y-%m-%d %H:%M` formatting aligned to extension cards. |
...
| TST-038 | `tests/test_cli_show_status_messages.py` + verifies CLI `show` freshness line uses provider `idle_time` timestamps with local-timezone `%Y-%m-%d %H:%M` parity, fail-state statistics suppression, retry-after rendering format, API-counter null→`0` rendering for `openai/openrouter/codex/geminiai`, and GeminiAI effective window `30d` under non-30d requests. |
