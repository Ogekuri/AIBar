...
- **REQ-020**: MUST include each discovered source symbol in `docs/REFERENCES.md` with file path, symbol kind, and line-range evidence; `@satisfies` tags MAY appear when available and MUST NOT be mandatory for requirement audit pass/fail.
...
- **TST-006**: MUST verify `req --here --references` reproduces `docs/REFERENCES.md` without missing symbol entries, preserves Doxygen field extraction, and confirms req-check audit does not fail solely for missing `@satisfies` tags.
- **TST-007**: MUST verify GNOME panel status labels render exactly one provider-colored bold `Err` for OAuth/rate-limit failures, hide normal percentage/cost labels in that state, keep provider order styling, verify icon thresholds, and preserve zero-cost currency rendering on non-error states.
...
| REQ-020 | `docs/REFERENCES.md` + per-symbol entries containing symbol identifier, file path, and line-range spans; `docs/REQUIREMENTS.md` + REQ-020 text marks `@satisfies` as optional for audit pass/fail. |
...
| TST-006 | `tests/test_references_inventory.py` + validates references inventory completeness/line ranges and asserts requirements text encodes non-mandatory `@satisfies` audit behavior. |
