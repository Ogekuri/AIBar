...
- **REQ-056**: `setup` and `login --provider geminiai` MUST execute OAuth installed-app authorization with scopes `https://www.googleapis.com/auth/monitoring.read` and `https://www.googleapis.com/auth/bigquery.readonly`, then persist refresh-capable authorized-user credentials.
