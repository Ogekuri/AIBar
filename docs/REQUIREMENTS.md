...
- **REQ-036**: CLI text `show` MUST suppress statistics lines (`Usage`, percentage bars, `Resets in`, `Remaining credits`, request/token/cost values) when provider/window status is `FAIL`, rendering only freshness and failure diagnostics.
...
| REQ-017 | `src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js` + `_parseOutput/_updateUI/_populateProviderCard` + freshness label uses `idle_time.<provider>.last_success_timestamp` and `idle_time.<provider>.idle_until_timestamp` formatted in local timezone `%Y-%m-%d %H:%M`. |
...
| REQ-036 | `src/aibar/aibar/cli.py` + `_build_result_panel` failed-state branch returns after error diagnostics and suppresses usage/reset/quota/cost statistics lines. |
...
| TST-004 | `tests/test_extension_quota_label.py` + popup-label assertions (`AIBar`, `Open AIBar Report`), failed-state metric-row suppression assertions, explicit error+retry metadata assertions, and `Updated/Next` provider-card label assertions sourced from `idle_time` timestamps with local-timezone `%Y-%m-%d %H:%M`; `tests/test_extension_dev_script.py` + `MUTTER_DEBUG_DUMMY_MODE_SPECS=1024x800`. |
...
| TST-038 | `tests/test_cli_show_status_messages.py` + verifies CLI `show` freshness line uses provider `idle_time` timestamps with local-timezone `%Y-%m-%d %H:%M` parity, fail-state statistics suppression, and retry-after rendering format. |
