...
- `Internal Call-Trace Tree`
  - `main(...)`: CLI command router [`src/aibar/aibar/cli.py`]
    - `show(...)`: usage fetch/report route with shared cache pipeline [`src/aibar/aibar/cli.py`]
      ...
      - `retrieve_results_via_cache_pipeline(...)`: shared retrieval order (`force` -> idle-time gate -> conditional refresh -> effective payload projection) using canonical cache sections `payload`/`status` [`src/aibar/aibar/cli.py`]
        ...
        - `_refresh_and_persist_cache_payload(...)`: conditional live refresh + status/payload merge write into `cache.json` [`src/aibar/aibar/cli.py`]
          ...
          - `save_cli_cache(...)`: persist sanitized cache document (`payload` + `status`) to `cache.json` only when merged payload content changed [`src/aibar/aibar/config.py`]
            ...
            - `save_idle_time(...)`: persist idle-time timestamps with blocking lock-file coordination [`src/aibar/aibar/config.py`]
      ...
      - `_print_result(...)`: ANSI provider-colored terminal panel rendering with provider-colored progress bars and full metrics/status coverage using `metrics.currency_symbol` for costs [`src/aibar/aibar/cli.py`]
        - `_provider_display_name(...)`: provider display-title resolver with GeminiAI title-case override [`src/aibar/aibar/cli.py`]
        - `_emit_provider_panel(...)`: provider-color border/padding/wrap renderer with ANSI-aware visible-width alignment for CLI provider panels [`src/aibar/aibar/cli.py`]
...
- `Internal Call-Trace Tree`
  - `AIBarExtension.enable(...)`: extension enable adapter; registers indicator via `Main.panel.addToStatusArea(this.uuid, ...)` [`src/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
    - `AIBarIndicator._init(...)`: indicator runtime bootstrap with panel title `AIBar Monitor` [`src/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
      ...
      - `AIBarIndicator._buildPanelButton(...)`: panel icon/status-label setup with ordered percentage+cost labels (Claude 5h, Claude 7d, Claude cost, OpenRouter cost, Copilot 30d, Codex 5h, Codex 7d, Codex cost, GeminiAI cost) and provider style classes [`src/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
      ...
      - `AIBarIndicator._refreshData(forceRefresh = false)`: subprocess-based JSON refresh with optional `--force` argument injection [`src/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
        ...
        - callback chain
          ...
          - `AIBarIndicator._updateUI(...)`: provider tab/card rendering update with deterministic order (`claude`, `openrouter`, `copilot`, `codex`, `geminiai`) plus ordered panel status-label projection (`claude5h`, `claude7d`, `claudeCost`, `openrouterCost`, `copilot`, `codex5h`, `codex7d`, `codexCost`, `geminiaiCost`) with hidden-label fallback for missing metrics; resolves window-scoped status entries from cache `status` section; per-card `Update at: HH:MM` label bottom-right populated from `data.updated_at`; updates icon threshold colors and >90% blink state [`src/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
