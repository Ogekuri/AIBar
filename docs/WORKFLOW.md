...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections, while default-window Claude/Codex text rendering emits one grouped panel per provider containing `5h` and `7d` sections [`src/aibar/aibar/cli.py`]
      ...
      - `_build_result_panel(...)`: deterministic title/body builder for one provider result before panel emission; renders `Updated: <datetime>, Next: <datetime>` in runtime local timezone using provider idle-time timestamps (`last_success_timestamp`, `idle_until_timestamp`) with payload-based fallback when idle-time state is unavailable; on `FAIL`, suppresses statistics lines and renders error diagnostics only; on `OK`, providers `openai/openrouter/codex/geminiai` always render `Requests` and `Tokens` with null counters normalized to `0` [`src/aibar/aibar/cli.py`]
      - `_build_dual_window_panel(...)`: grouped Claude/Codex panel composer that merges `5h` and `7d` window lines into one provider panel with blank-line-separated sections and shared-line deduplication [`src/aibar/aibar/cli.py`]
...
- `Internal Call-Trace Tree`
  - `AIBarExtension.enable(...)`: extension enable adapter; registers indicator via `Main.panel.addToStatusArea(this.uuid, ...)` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
    - `AIBarIndicator._init(...)`: indicator runtime bootstrap with panel title `AIBar Monitor` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
      - provider-order initialization: `_providerOrder = ['claude', 'openrouter', 'copilot', 'codex', 'geminiai']` for deterministic tab/card sorting [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
        ...
        - callback chain
          - `AIBarIndicator._parseOutput(...)`: JSON decode + state update supporting canonical `payload`/`status`/`idle_time`/`freshness`/`extension` schema; reads provider freshness from `freshness` (fallback `idle_time`), reads `extension.gnome_refresh_interval_seconds`, and reschedules auto-refresh timer on interval change [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
            ...
            - `AIBarIndicator._updateProviderCard(...)`: card lifecycle update with optional window status entry argument [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
              - `AIBarIndicator._createProviderCard(...)`: card widget graph creation including `updateAtLabel` bottom-right row with explicit `Clutter.ActorAlign.END` alignment and `aibar-reset-label` font/color parity [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
