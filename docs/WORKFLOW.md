...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections, while text output sorts provider panels as `claude/openrouter/copilot/codex/openai/geminiai`, omits window-heading rows, prints only `Error: <reason>` on `FAIL`, and right-aligns trailing `Updated/Next` on `OK` panels [`src/aibar/aibar/cli.py`]
      ...
      - `_build_result_panel(...)`: deterministic title/body builder for one provider result before panel emission; on `FAIL`, returns only `Error: <reason>` without status/window/freshness/statistics rows; on `OK`, renders `Status` first, detail rows without `Window <window>` headings, and trailing `Updated: <datetime>, Next: <datetime>` freshness line in runtime local timezone, plus cost/counter rows with existing provider-specific rules; GeminiAI billing-service line appends parenthesized human-readable service descriptions with truncation marker `...` when hidden entries remain [`src/aibar/aibar/cli.py`]
        - `_format_billing_service_descriptions(...)`: summarize ordered GeminiAI billing service descriptions into comma-separated preview text capped to three names with `...` suffix on truncation [`src/aibar/aibar/cli.py`]
