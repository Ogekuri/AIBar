...
- `Internal Call-Trace Tree`
  - `main(...)`: CLI command router [`src/aibar/aibar/cli.py`]
    - `show(...)`: usage fetch/report route with per-provider TTL cache [`src/aibar/aibar/cli.py`]
      ...
      - `_fetch_claude_dual(...)`: single-API-call dual-window fetch for Claude with rate-limit cooldown and cross-window raw re-parse fallback on both the cooldown pre-check path and the post-fetch 429 path [`src/aibar/aibar/cli.py`]
