...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline [`src/aibar/aibar/cli.py`]
      ...
      - `retrieve_results_via_cache_pipeline(...)`: shared `show` process flow: (1) idle-time check, (2) if refresh needed: load cache → modular API calls → in-memory merge → locked cache write → idle-time update, (3) if no refresh: single locked cache read, (4) decode results for presentation; at most one `load_cli_cache` per execution [`src/aibar/aibar/cli.py`]
        ...
        - `_refresh_and_persist_cache_payload(...)`: execute modular API calls for expired providers, merge results in memory, then persist cache.json and idle-time.json under lock in a single write point [`src/aibar/aibar/cli.py`]
          ...
          - `_update_idle_time_after_refresh(...)`: persist provider-scoped idle-time metadata from refresh outcomes, applying `idle_until = max(current_time + retry_after, current_time + idle_delay)` for failures [`src/aibar/aibar/cli.py`]
            ...
            - `build_idle_time_state(...)`: normalize provider-local idle timestamps into canonical epoch/ISO fields [`src/aibar/aibar/config.py`]
      ...
      - `_build_result_panel(...)`: deterministic title/body builder for one provider result before panel emission; renders `Updated: <datetime>, Next: <datetime>` in runtime local timezone using `%Y-%m-%d %H:%M`; on `FAIL`, suppresses statistics lines and renders error diagnostics only [`src/aibar/aibar/cli.py`]
  ...
  - `AIBarExtension.enable(...)`: extension enable adapter; registers indicator via `Main.panel.addToStatusArea(this.uuid, ...)` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
    - `AIBarIndicator._init(...)`: indicator runtime bootstrap with panel title `AIBar Monitor` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
      ...
      - `AIBarIndicator._refreshData(forceRefresh = false)`: subprocess-based JSON refresh with optional `--force` argument injection [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
        ...
        - callback chain
          - `AIBarIndicator._parseOutput(...)`: JSON decode + state update supporting canonical `payload`/`status`/`extension` schema; reads `extension.gnome_refresh_interval_seconds` and reschedules auto-refresh timer on interval change [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
            - `AIBarIndicator._updateUI(...)`: provider tab/card rendering update with deterministic order (`claude`, `openrouter`, `copilot`, `codex`, `geminiai`) plus ordered panel status-label projection (`claude5h`, `claude7d`, `claudeCost`, `openrouterCost`, `copilot`, `codex5h`, `codex7d`, `codexCost`, `openaiCost`, `geminiaiCost`) with hidden-label fallback for missing metrics and zero-cost visibility via `_panelCostText(...)`; resolves window-scoped status entries from cache `status` section and forwards them to card rendering; per-card `Updated: <datetime>, Next: <datetime>` label bottom-right populated from `data.updated_at` and `this._refreshIntervalSeconds` via local-time formatter `%Y-%m-%d %H:%M`; updates icon threshold colors and >90% blink state [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
