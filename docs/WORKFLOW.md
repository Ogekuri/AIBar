...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections, while text output sorts provider panels as `claude/openrouter/copilot/codex/openai/geminiai`, omits window-heading rows, prints only `Error: <reason>` on `FAIL`, and right-aligns trailing `Updated/Next` on `OK` panels [`src/aibar/aibar/cli.py`]
      ...
      - `_build_result_panel(...)`: deterministic title/body builder for one provider result before panel emission; on `FAIL`, returns only `Error: <reason>` without status/window/freshness/statistics rows; on `OK`, renders `Status` first, detail rows without `Window <window>` headings, and trailing `Updated: <datetime>, Next: <datetime>` freshness line in runtime local timezone, plus cost/counter rows with existing provider-specific rules; GeminiAI billing-service line appends parenthesized human-readable service descriptions including all discovered services in source order [`src/aibar/aibar/cli.py`]
      ...
      - `_build_dual_window_panel(...)`: grouped Claude/Codex panel composer that returns only one error row on any `FAIL`; on `OK`, renders one panel with `Status` first, deduplicated shared details, combined `5h`/`7d` details without heading labels, shared `Remaining credits` from `7d`, optional CODEX `Cost/Requests/Tokens` footer rows, and trailing `Updated: ..., Next: ...` freshness line [`src/aibar/aibar/cli.py`]
        ...
        - callback chain
          - `AIBarIndicator._parseOutput(...)`: JSON decode + state update supporting canonical `payload`/`status`/`idle_time`/`freshness`/`extension` schema; reads provider freshness from `freshness` (fallback `idle_time`), reads `extension.gnome_refresh_interval_seconds` plus `extension.idle_delay_seconds`, and reschedules auto-refresh timer on interval change [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
            ...
            - `AIBarIndicator._updateProviderCard(...)`: card lifecycle update with optional window status entry argument [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
              ...
              - `AIBarIndicator._populateProviderCard(...)`: metrics-to-widget projection with cache-status error precedence (`statusEntry.error` over payload `data.error`); failed states render only `Error: <message>`, hide status/window/freshness/usage/reset/quota/cost rows and percentage bars; successful states render provider metrics with existing card rules, including hidden window-label text for bars, cost/counter rows, and quota/reset formatting [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
