...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher with execution start/end logging [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections (including Copilot overage unit-cost config), while text output sorts provider panels as `claude/openrouter/copilot/codex/openai/geminiai`, omits window-heading rows, renders `FAIL` blocks as `Status: FAIL` + `Reason: <reason>`, and right-aligns trailing `Updated/Next` on both `OK` and `FAIL` panels [`src/aibar/aibar/cli.py`]
      ...
      - `_build_result_panel(...)`: deterministic title/body builder for one provider result before panel emission; on `FAIL`, returns `Status: FAIL`, blank separator, `Reason: <reason>`, blank separator, and trailing `Updated: <datetime>, Next: <datetime>` freshness line; on `OK`, renders `Status` first, detail rows without `Window <window>` headings, and trailing `Updated: <datetime>, Next: <datetime>` freshness line in runtime local timezone, plus cost/counter rows with existing provider-specific rules; Copilot `OK` rows render `Cost: <currency><value>` from `metrics.cost`, with fallback overage computed from `remaining` semantics; GeminiAI billing-service line appends parenthesized human-readable service descriptions including all discovered services in source order [`src/aibar/aibar/cli.py`]
