...
- `Internal Call-Trace Tree`
  - `main(...)`: CLI command router [`src/aibar/aibar/cli.py`]
    - `show(...)`: usage fetch/report route with per-provider TTL cache [`src/aibar/aibar/cli.py`]
      ...
      - `_fetch_claude_dual(...)`: single-API-call dual-window fetch for Claude [`src/aibar/aibar/cli.py`]
        ...
        - `ClaudeOAuthProvider.fetch_all_windows(...)`: single HTTP request for multiple windows [`src/aibar/aibar/providers/claude_oauth.py`]
          - `ClaudeOAuthProvider._request_usage(...)`: HTTP GET with retry on 429 [`src/aibar/aibar/providers/claude_oauth.py`]
        ...
        - `ResultCache.set(...)`: cache store after fetch [`src/aibar/aibar/cache.py`]
        - `ResultCache.get_last_good(...)`: last-known-good fallback on error [`src/aibar/aibar/cache.py`]
      - `_fetch_result(...)`: provider fetch with cache lookup, store, and last-good fallback [`src/aibar/aibar/cli.py`]
        - `ResultCache.get(...)`: cache lookup before API call [`src/aibar/aibar/cache.py`]
