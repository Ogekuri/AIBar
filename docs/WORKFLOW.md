...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections, while text output sorts provider panels as `claude/openrouter/copilot/codex/openai/geminiai` and keeps grouped default-window Claude/Codex sections [`src/aibar/aibar/cli.py`]
      ...
      - `_build_result_panel(...)`: deterministic title/body builder for one provider result before panel emission; renders `Status` first, then a blank separator, then window-specific details, then a blank separator and trailing `Updated: <datetime>, Next: <datetime>` freshness line in runtime local timezone using provider idle-time timestamps (`last_success_timestamp`, `idle_until_timestamp`) with payload fallback; on `FAIL`, suppresses statistics lines and renders only error diagnostics; on `OK`, providers `openai/openrouter/codex/geminiai` always render `Requests` and `Tokens` with null counters normalized to `0` [`src/aibar/aibar/cli.py`]
      - `_build_dual_window_panel(...)`: grouped Claude/Codex panel composer that renders one panel with `Status` first, optional deduplicated shared details, blank-line-separated `5h`/`7d` sections, CODEX `Cost/Requests/Tokens` footer lines after `7d`, and trailing `Updated: ..., Next: ...` freshness line [`src/aibar/aibar/cli.py`]
      ...
      - `AIBarIndicator._refreshData(forceRefresh = false)`: subprocess-based JSON refresh with optional `--force` argument injection [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
        ...
        - callback chain
          - `AIBarIndicator._parseOutput(...)`: JSON decode + state update supporting canonical `payload`/`status`/`idle_time`/`freshness`/`extension` schema; reads provider freshness from `freshness` (fallback `idle_time`), reads `extension.gnome_refresh_interval_seconds`, and reschedules auto-refresh timer on interval change [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
            - `AIBarIndicator._updateUI(...)`: provider tab/card rendering update with deterministic order (`claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`) plus ordered panel status-label projection (`claude5h`, `claude7d`, `claudeCost`, `openrouterCost`, `copilot`, `codex5h`, `codex7d`, `codexCost`, `openaiCost`, `geminiaiCost`) with hidden-label fallback for missing metrics and zero-cost visibility via `_panelCostText(...)`; resolves window-scoped status entries from cache `status` section and provider-scoped freshness entries via `_resolveProviderFreshnessState(...)` (canonical `freshness`/`idle_time` with status-based fallback), then forwards both to card rendering; per-card `Updated: <datetime>, Next: <datetime>` label bottom-right inside provider card container populated from resolved freshness timestamps via local-time formatter `%Y-%m-%d %H:%M`; updates icon threshold colors and >90% blink state [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
