...
- `id: THR:chrome-ext#refresh-alarm`
  ...
  - `defining_files: src/aibar/chrome-extension/background.js`
...
### PROC:chrome-ext
- `Entrypoints`
  - `_initializeRuntime(...)`: service-worker startup bootstrap [`src/aibar/chrome-extension/background.js`]
...
- `Internal Call-Trace Tree`
  - `_initializeRuntime(...)`: startup orchestrator [`src/aibar/chrome-extension/background.js`]
    ...
    - `_refreshAllProviders(...)`: ordered provider refresh execution [`src/aibar/chrome-extension/background.js`]
      ...
      - `_assertProviderPayloadUsable(...)`: parser usability gate [`src/aibar/chrome-extension/background.js`]
        - `providerPayloadHasUsableMetrics(...)`: quota/progress presence validation [`src/aibar/chrome-extension/parsers.js`]
      - `parseCodexUsageHtml(...)`: Codex metric extraction from HTML, bootstrap JSON, and escaped script key/value artifacts [`src/aibar/chrome-extension/parsers.js`]
      - `_assertProviderPayloadUsable(...)`: parser usability gate [`src/aibar/chrome-extension/background.js`]
        - `providerPayloadHasUsableMetrics(...)`: quota/progress presence validation [`src/aibar/chrome-extension/parsers.js`]
      - `parseCopilotFeaturesHtml(...)`: Copilot-features extraction [`src/aibar/chrome-extension/parsers.js`]
      - `parseCopilotPremiumHtml(...)`: Copilot-premium extraction [`src/aibar/chrome-extension/parsers.js`]
      - `mergeCopilotPayloads(...)`: Copilot source merge [`src/aibar/chrome-extension/parsers.js`]
      - `_assertProviderPayloadUsable(...)`: parser usability gate [`src/aibar/chrome-extension/background.js`]
        - `providerPayloadHasUsableMetrics(...)`: quota/progress presence validation [`src/aibar/chrome-extension/parsers.js`]
  ...
  - `_handleMessage(...)`: popup RPC route [`src/aibar/chrome-extension/background.js`]
    ...
    - `_buildDebugExecuteResponse(...)`: debug command dispatcher wrapper [`src/aibar/chrome-extension/background.js`]
      ...
      - `_buildDebugHttpResponse(...)`: bounded head/tail preview + hash metadata [`src/aibar/chrome-extension/background.js`]
        - `_sha256Hex(...)`: deterministic response-body digest [`src/aibar/chrome-extension/background.js`]
        - `_buildHtmlProbe(...)`: HTML classification probe [`src/aibar/chrome-extension/background.js`]
      - `_executeProviderDiagnoseCommand(...)`: provider/page diagnose execution [`src/aibar/chrome-extension/background.js`]
        - `_buildWindowAssignmentDiagnostics(...)`: parser window-selection trace envelope [`src/aibar/chrome-extension/background.js`]
          - `extractWindowAssignmentDiagnostics(...)`: per-window candidate ranking trace extraction [`src/aibar/chrome-extension/parsers.js`]
        - `_buildPayloadAssertion(...)`: non-throwing parser usability assertion wrapper [`src/aibar/chrome-extension/background.js`]
      - `_executeProvidersPagesGetCommand(...)`: fixed provider-page + related-resource diagnostics execution [`src/aibar/chrome-extension/background.js`]
        - `_executeProviderPageDownload(...)`: provider page fetch + parser analysis envelope [`src/aibar/chrome-extension/background.js`]
        - `_downloadRelatedResources(...)`: same-origin related-content fetch loop [`src/aibar/chrome-extension/background.js`]
          - `_extractRelatedResourceUrls(...)`: script/link related URL extraction [`src/aibar/chrome-extension/background.js`]
        - `_normalizeDebugRelatedLimit(...)`: related-resource cap normalization [`src/aibar/chrome-extension/background.js`]
        - `_buildProviderPayloadAnalysis(...)`: parser usability/quality summary [`src/aibar/chrome-extension/background.js`]
...
- `Entrypoints`
  - `chrome.alarms.onAlarm` callback: periodic refresh trigger [`src/aibar/chrome-extension/background.js`]
- `Lifecycle/Trigger`
  - Activated whenever alarm `aibar-refresh` fires.
  - Invokes one ordered provider refresh cycle and publishes updated state.
- `Internal Call-Trace Tree`
  - alarm callback: periodic trigger [`src/aibar/chrome-extension/background.js`]
    - `_refreshAllProviders(...)`: ordered download + parse + persist pipeline [`src/aibar/chrome-extension/background.js`]
      - `_fetchHtml(...)`: page download [`src/aibar/chrome-extension/background.js`]
      - `parseClaudeUsageHtml(...)`: parser execution [`src/aibar/chrome-extension/parsers.js`]
      - `parseCodexUsageHtml(...)`: parser execution for HTML/bootstrap/escaped-script extraction [`src/aibar/chrome-extension/parsers.js`]
      - `parseCopilotFeaturesHtml(...)`: parser execution [`src/aibar/chrome-extension/parsers.js`]
      - `parseCopilotPremiumHtml(...)`: parser execution [`src/aibar/chrome-extension/parsers.js`]
      - `mergeCopilotPayloads(...)`: parser merge execution [`src/aibar/chrome-extension/parsers.js`]
      - `_persistState(...)`: storage synchronization [`src/aibar/chrome-extension/background.js`]
- `External Boundaries`
...
- `Internal Call-Trace Tree`
  - `main(...)`: CLI command router [`src/aibar/aibar/cli.py`]
    - `show(...)`: usage fetch/report route with per-provider TTL cache [`src/aibar/aibar/cli.py`]
      ...
      - `_fetch_claude_dual(...)`: single-API-call dual-window fetch for Claude without TTL cache reuse and HTTP-429 partial-window normalization using persisted snapshot payloads [`src/aibar/aibar/cli.py`]
        ...
        - `_persist_claude_dual_snapshot(...)`: store latest successful Claude dual-window raw payload under cache home for future 429 fallback [`src/aibar/aibar/cli.py`]
      ...
      - `_fetch_result(...)`: provider fetch with non-Claude cache lookup/store/fallback and Claude fresh-fetch bypass [`src/aibar/aibar/cli.py`]
        - `ResultCache.get(...)`: cache lookup before API call for non-Claude providers [`src/aibar/aibar/cache.py`]
          - `ResultCache._is_cacheable_provider(...)`: provider cache-policy gate (Claude returns non-cacheable) [`src/aibar/aibar/cache.py`]
        - `ResultCache.is_rate_limited(...)`: rate-limit cooldown check for non-Claude providers [`src/aibar/aibar/cache.py`]
          - `ResultCache._is_cacheable_provider(...)`: provider cache-policy gate (Claude returns non-cacheable) [`src/aibar/aibar/cache.py`]
        - `ResultCache.get_last_good(...)`: last-known-good fallback during cooldown for non-Claude providers [`src/aibar/aibar/cache.py`]
          - `ResultCache._is_cacheable_provider(...)`: provider cache-policy gate (Claude returns non-cacheable) [`src/aibar/aibar/cache.py`]
        ...
        - `ResultCache.set(...)`: cache store after fetch for non-Claude providers; Claude path performs no-op persistence and cooldown writes [`src/aibar/aibar/cache.py`]
          - `ResultCache._is_cacheable_provider(...)`: provider cache-policy gate (Claude returns non-cacheable) [`src/aibar/aibar/cache.py`]
        ...
        - `ResultCache.get_last_good(...)`: last-known-good fallback on error for non-Claude providers [`src/aibar/aibar/cache.py`]
          - `ResultCache._is_cacheable_provider(...)`: provider cache-policy gate (Claude returns non-cacheable) [`src/aibar/aibar/cache.py`]
