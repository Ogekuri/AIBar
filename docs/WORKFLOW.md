...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections (`extension` includes `window_labels` for GNOME bar labels), while text output sorts provider panels as `claude/openrouter/copilot/codex/openai/geminiai`, renders window headings as `Window <window>:`, right-aligns trailing `Updated/Next`, and keeps grouped default-window Claude/Codex sections [`src/aibar/aibar/cli.py`]
      ...
      - `retrieve_results_via_cache_pipeline(...)`: shared `show` process flow: (1) idle-time check, (2) if refresh needed: load cache → modular API calls → in-memory merge → locked cache write → idle-time update, (3) if no refresh: single locked cache read, (4) decode results for presentation; at most one `load_cli_cache` per execution [`src/aibar/aibar/cli.py`]
      ...
      - `_build_result_panel(...)`: deterministic title/body builder for one provider result before panel emission; renders `Status` first, then a blank separator, then `Window <window>:` details, then a blank separator and trailing `Updated: <datetime>, Next: <datetime>` freshness line in runtime local timezone using provider idle-time timestamps (`last_success_timestamp`, `idle_until_timestamp`) with payload fallback; on `FAIL`, suppresses statistics lines and renders only error diagnostics; on `OK`, renders `Cost: <value>` with bright-white bold value, renders `Requests: <n>` and `Tokens: <total> (<in> in / <out> out)` with null counters normalized to `0`, and inserts one blank line before GeminiAI `Billing table` row [`src/aibar/aibar/cli.py`]
      - `_build_dual_window_panel(...)`: grouped Claude/Codex panel composer that renders one panel with `Status` first, optional deduplicated shared details, blank-line-separated `Window 5h:`/`Window 7d:` sections, one shared `Remaining credits` line derived from `7d` appended after the `Window 7d:` section with a blank separator, CODEX `Cost/Requests/Tokens` footer lines after that shared quota row, and trailing `Updated: ..., Next: ...` freshness line [`src/aibar/aibar/cli.py`]
        ...
        - callback chain
          - `AIBarIndicator._parseOutput(...)`: JSON decode + state update supporting canonical `payload`/`status`/`idle_time`/`freshness`/`extension` schema; reads provider freshness from `freshness` (fallback `idle_time`), reads `extension.gnome_refresh_interval_seconds` plus `extension.idle_delay_seconds`, parses `extension.window_labels` for fixed-window provider bar labels, and reschedules auto-refresh timer on interval change [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
            ...
            - `AIBarIndicator._updateProviderCard(...)`: card lifecycle update with optional window status entry argument [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
              ...
              - `AIBarIndicator._populateProviderCard(...)`: metrics-to-widget projection with cache-status error precedence (`statusEntry.error` over payload `data.error`); failed states render only `Status: FAIL`, `Window: <window>`, `Error: <message>`, and optional normalized `HTTP status`/`Retry after` diagnostics, hide usage/reset/quota/cost rows and percentage bars, and preserve `Updated: <datetime>, Next: <datetime>` freshness label from `freshnessState.last_success_timestamp` and `freshnessState.idle_until_timestamp` through local formatter `_formatLocalDateTime(...)`; successful states render single-window bars for `copilot/openrouter/openai/geminiai` with left labels sourced from parsed `extension.window_labels` (fallback `30d`), render `Costs: <value>` with bold bright-white numeric values (currency included), hide empty BYOK rows so OpenRouter/OpenAI/GeminiAI do not render spacer lines after `Costs:`, render Codex/Claude/Copilot `Remaining credits: <remaining> / <limit>` with bright-white bold remaining value, preserve provider-colored progress fills with `⚠️ Limit reached!` suffix at displayed `100.0%` for Claude/Codex/Copilot bars, and force `openai/openrouter/codex/geminiai` labels `Requests: <n>` plus `Tokens: <total> (<in> in / <out> out)` with null counters normalized to `0` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
                ...
                - `_buildHttpStatusRetryLabel(...)`: normalized `HTTP status`/`Retry after` diagnostics formatter (`HTTP status: <code>, Retry after: <seconds> sec.`) [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
