...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher with execution start/end logging [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections (including Copilot overage unit-cost config), while text output sorts provider panels as `claude/openrouter/copilot/codex/openai/geminiai`, omits window-heading rows, renders `FAIL` blocks as `Status: FAIL` + `Reason: <reason>`, and right-aligns trailing `Updated/Next` on both `OK` and `FAIL` panels [`src/aibar/aibar/cli.py`]
        ...
        - `_refresh_and_persist_cache_payload(...)`: execute modular API calls for expired providers, merge results in memory, persist cache/idle updates, and append refresh-branch runtime log rows; Claude dual-window refresh path appends provider failure runtime logs for both `5h` and `7d` results so OAuth/429 failures always emit retry-after or unavailability evidence [`src/aibar/aibar/cli.py`]
          ...
          - `_fetch_result(...)`: throttled provider fetch wrapper with API-call start/end/error runtime logging, OAuth/rate-limit failure-category logging, and debug-result logging [`src/aibar/aibar/cli.py`]
            ...
            - `OpenRouterUsageProvider.fetch(...)`: OpenRouter usage retrieval with `get_api_call_timeout_seconds` for HTTP timeout and `resolve_currency_symbol` for `currency_symbol` [`src/aibar/aibar/providers/openrouter.py`]
              - `get_api_call_timeout_seconds(...)`: resolve HTTP timeout from `RuntimeConfig.api_call_timeout_milliseconds` [`src/aibar/aibar/config.py`]
              - `resolve_currency_symbol(...)`: resolve ISO code or symbol from API raw → configured default → `"$"` [`src/aibar/aibar/config.py`]
            - `CopilotProvider.fetch(...)`: Copilot usage retrieval with `get_api_call_timeout_seconds` for HTTP timeout and `resolve_currency_symbol` for `currency_symbol`; normalized payload computes/persists `premium_requests_extra_cost` from premium-request overage and runtime-configured unit cost [`src/aibar/aibar/providers/copilot.py`]
              - `get_api_call_timeout_seconds(...)`: resolve HTTP timeout from `RuntimeConfig.api_call_timeout_milliseconds` [`src/aibar/aibar/config.py`]
              - `resolve_currency_symbol(...)`: resolve ISO code or symbol from API raw → configured default → `"$"` [`src/aibar/aibar/config.py`]
      ...
      - `_build_result_panel(...)`: deterministic title/body builder for one provider result before panel emission; on `FAIL`, returns `Status: FAIL`, blank separator, `Reason: <reason>`, blank separator, and trailing `Updated: <datetime>, Next: <datetime>` freshness line; on `OK`, renders `Status` first, detail rows without `Window <window>` headings, and trailing `Updated: <datetime>, Next: <datetime>` freshness line in runtime local timezone, plus cost/counter rows with existing provider-specific rules; Copilot `OK` rows append `Extra premium cost: <currency><value>` from cached overage metadata; GeminiAI billing-service line appends parenthesized human-readable service descriptions including all discovered services in source order [`src/aibar/aibar/cli.py`]
