...
- `Internal Call-Trace Tree`
  - `main(...)`: CLI command router [`src/aibar/aibar/cli.py`]
    - `show(...)`: usage fetch/report route with per-provider TTL cache [`src/aibar/aibar/cli.py`]
      ...
      - `_fetch_claude_dual(...)`: single-API-call dual-window fetch for Claude without cache reuse and HTTP-429 partial-window normalization [`src/aibar/aibar/cli.py`]
        ...
        - `_build_claude_rate_limited_partial_result(...)`: synthetic 100%-usage + reset projection for Claude 429 rendering continuity [`src/aibar/aibar/cli.py`]
