...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections, while text output sorts provider panels as `claude/openrouter/copilot/codex/openai/geminiai`, renders window headings as `Window <window>:`, right-aligns trailing `Updated/Next`, and keeps grouped default-window Claude/Codex sections [`src/aibar/aibar/cli.py`]
