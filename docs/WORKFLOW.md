...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections, while text output sorts provider panels as `claude/openrouter/copilot/codex/openai/geminiai` and keeps grouped default-window Claude/Codex sections [`src/aibar/aibar/cli.py`]
      ...
      - GeminiAI window normalization branch: when `show --provider geminiai` is selected, effective window is forced to `30d` before retrieval pipeline execution, ignoring explicit non-`30d` window arguments [`src/aibar/aibar/cli.py`]
      ...
      - `retrieve_results_via_cache_pipeline(...)`: shared `show` process flow: (1) idle-time check, (2) if refresh needed: load cache → modular API calls → in-memory merge → locked cache write → idle-time update, (3) if no refresh: single locked cache read, (4) decode results for presentation; at most one `load_cli_cache` per execution [`src/aibar/aibar/cli.py`]
        ...
        - `_load_cached_results(...)`: cached payload decode/project helper with provider/window `FAIL` overlays from cache `status` metadata [`src/aibar/aibar/cli.py`]
          - `_project_cached_window(...)`: provider parser projection to requested window; enforces fixed-window canonicalization for `copilot`/`geminiai` (`30d`) when cached payload window is stale [`src/aibar/aibar/cli.py`]
  ...
  - `AIBarExtension.enable(...)`: extension enable adapter; registers indicator via `Main.panel.addToStatusArea(this.uuid, ...)` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
    - `AIBarIndicator._init(...)`: indicator runtime bootstrap with panel title `AIBar Monitor` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
      ...
      - `AIBarIndicator._refreshData(forceRefresh = false)`: subprocess-based JSON refresh with optional `--force` argument injection [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
        ...
        - callback chain
          - `AIBarIndicator._parseOutput(...)`: JSON decode + state update supporting canonical `payload`/`status`/`idle_time`/`freshness`/`extension` schema; reads provider freshness from `freshness` (fallback `idle_time`), reads `extension.gnome_refresh_interval_seconds`, and reschedules auto-refresh timer on interval change [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
            - `AIBarIndicator._updateUI(...)`: provider tab/card rendering update with deterministic order (`claude`, `openrouter`, `copilot`, `codex`, `openai`, `geminiai`) plus ordered panel status-label projection (`claude5h`, `claude7d`, `claudeCost`, `openrouterCost`, `copilot`, `codex5h`, `codex7d`, `codexCost`, `openaiCost`, `geminiaiCost`) with hidden-label fallback for missing metrics and zero-cost visibility via `_panelCostText(...)`; resolves window-scoped status entries from cache `status` section and provider-scoped freshness entries via `_resolveProviderFreshnessState(...)` (canonical `freshness`/`idle_time`, status-derived fallback only when freshness is absent), then forwards both to card rendering; per-card `Updated: <datetime>, Next: <datetime>` label bottom-right inside provider card container populated from resolved freshness timestamps via local-time formatter `%Y-%m-%d %H:%M`; updates icon threshold colors and >90% blink state [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
            ...
            - `AIBarIndicator._updateProviderCard(...)`: card lifecycle update with optional window status entry argument [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
              ...
              - `AIBarIndicator._populateProviderCard(...)`: metrics-to-widget projection with cache-status error precedence (`statusEntry.error` over payload `data.error`); failed states render only `Status: FAIL`, `Window: <window>`, `Error: <message>`, and optional normalized `HTTP status`/`Retry after` diagnostics, hide usage/reset/quota/cost rows and percentage bars, and preserve `Updated: <datetime>, Next: <datetime>` freshness label from `freshnessState.last_success_timestamp` and `freshnessState.idle_until_timestamp` through local formatter `_formatLocalDateTime(...)`; successful states render `Cost: <value>` with bright-white bold value (OpenAI/OpenRouter/Codex/GeminiAI and any provider with cost metric), render Codex/Claude/Copilot `Remaining credits: <remaining> / <limit>` with bright-white bold remaining value, preserve provider-colored progress fills with `⚠️ Limit reached!` suffix at displayed `100.0%` for Claude/Codex/Copilot bars, and force `openai/openrouter/codex/geminiai` labels `Requests: <n>` plus `Tokens: <total> (<in> in / <out> out)` with null counters normalized to `0` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
                - `_resolveProviderFreshnessState(...)`: provider freshness resolver that prioritizes `freshness`/`idle_time` sections and falls back to status-derived timestamps [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
                  - `_buildFallbackFreshnessState(...)`: derives fallback `last_success_timestamp` and `idle_until_timestamp` from `statusEntry.updated_at` and active refresh interval [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
