    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline [`src/aibar/aibar/cli.py`]
      ...
      - `retrieve_results_via_cache_pipeline(...)`: shared retrieval order (`force` -> provider-scoped idle-time evaluation -> per-provider conditional refresh -> effective payload projection) using canonical cache sections `payload`/`status` [`src/aibar/aibar/cli.py`]
        ...
        - `_load_cached_results(...)`: cached payload decode/project helper with status-aware overlays for Claude 429 and GeminiAI cached `FAIL` status propagation [`src/aibar/aibar/cli.py`]
          ...
          - `_overlay_cached_failure_status(...)`: provider/window cached `FAIL` status overlay onto projected payload results so text rendering surfaces authentication and refresh-limit errors from cache status metadata [`src/aibar/aibar/cli.py`]
      ...
      - `_build_result_panel(...)`: deterministic title/body builder for one provider result before panel emission; renders `Updated: <datetime>, Next: <datetime>` in runtime local timezone using `%Y-%m-%d %H:%M` and preserves reset-line alignment semantics without warning-glyph suffixes [`src/aibar/aibar/cli.py`]
    ...
    - `AIBarIndicator._init(...)`: indicator runtime bootstrap with panel title `AIBar Monitor` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
      ...
      - `AIBarIndicator._refreshData(forceRefresh = false)`: subprocess-based JSON refresh with optional `--force` argument injection [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
        ...
        - callback chain
          ...
          - `AIBarIndicator._updateUI(...)`: provider tab/card rendering update with deterministic order (`claude`, `openrouter`, `copilot`, `codex`, `geminiai`) plus ordered panel status-label projection (`claude5h`, `claude7d`, `claudeCost`, `openrouterCost`, `copilot`, `codex5h`, `codex7d`, `codexCost`, `openaiCost`, `geminiaiCost`) with hidden-label fallback for missing metrics and zero-cost visibility via `_panelCostText(...)`; resolves window-scoped status entries from cache `status` section; per-card `Updated: <datetime>, Next: <datetime>` label bottom-right populated from `data.updated_at` and `this._refreshIntervalSeconds` via local-time formatter `%Y-%m-%d %H:%M`; updates icon threshold colors and >90% blink state [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
            ...
            - `AIBarIndicator._updateProviderCard(...)`: card lifecycle update with optional window status entry argument [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
              ...
              - `AIBarIndicator._populateProviderCard(...)`: metrics-to-widget projection with cache-status error precedence (`statusEntry.error` over payload `data.error`), quota label, cost label using `metrics.currency_symbol`, byok label using `metrics.currency_symbol`, provider-colored progress fills, `⚠️ Limit reached!` suffix at displayed `100.0%` for Claude/Codex/Copilot bars, and `Updated: <datetime>, Next: <datetime>` bottom-right label computed from `data.updated_at` plus `this._refreshIntervalSeconds` through local formatter `_formatLocalDateTime(...)` [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
