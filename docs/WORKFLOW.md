...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline [`src/aibar/aibar/cli.py`]
      ...
      - `retrieve_results_via_cache_pipeline(...)`: shared retrieval order (`force` -> provider-scoped idle-time evaluation -> per-provider conditional refresh -> effective payload projection) using canonical cache sections `payload`/`status` [`src/aibar/aibar/cli.py`]
        ...
        - `_refresh_and_persist_cache_payload(...)`: conditional live refresh + status/payload merge write into `cache.json` [`src/aibar/aibar/cli.py`]
          ...
          - `save_cli_cache(...)`: persist sanitized cache document (`payload` + `status`) to `cache.json` only when merged payload content changed [`src/aibar/aibar/config.py`]
            ...
            - `_update_idle_time_after_refresh(...)`: persist provider-scoped idle-time metadata from refresh outcomes, applying `idle_until = last_attempt_at + max(idle_delay_seconds, retry_after_seconds_or_0)` for failures [`src/aibar/aibar/cli.py`]
