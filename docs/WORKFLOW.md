          ...
          - `AIBarIndicator._parseOutput(...)`: JSON decode + state update supporting canonical `payload`/`status`/`idle_time`/`freshness`/`extension` schema; reads provider freshness from `freshness` (fallback `idle_time`), reads `extension.gnome_refresh_interval_seconds`, `extension.idle_delay_seconds`, and `extension.copilot_extra_premium_request_cost`, and reschedules auto-refresh timer on interval change [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
            ...
            - `AIBarIndicator._updateProviderCard(...)`: card lifecycle update with optional window status entry argument [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
              - `AIBarIndicator._createProviderCard(...)`: card widget graph creation including bottom-right `updateAtLabel` row with expanding right-aligned layout (`x_expand` + `Clutter.ActorAlign.END`), `aibar-reset-label` font/color parity, and per-bar over-limit marker/segment actors attached to progress backgrounds [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
                ...
                - `_attachOverLimitActors(...)`: attach progress-background marker and neutral over-limit fill actors for shared geometry updates [`src/aibar/aibar/gnome-extension/aibar@aibar.panel/extension.js`]
