...
- `Internal Call-Trace Tree`
  - `StartupPreflightGroup.main(...)`: startup preflight wrapper around Click parser/dispatcher [`src/aibar/aibar/cli.py`]
    ...
    - `show(...)`: usage fetch/report route with shared cache pipeline; JSON branch emits canonical `payload`/`status` plus `idle_time`/`freshness`/`extension` sections [`src/aibar/aibar/cli.py`]
      ...
      - `_overlay_cached_failure_status(...)`: apply cache `status` section `FAIL` overlays to both projected dual-window render results (`5h`, `7d`) before panel emission so each window preserves independent cached auth/rate-limit diagnostics [`src/aibar/aibar/cli.py`]
