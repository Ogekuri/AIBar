  ...
  - `main(...)`: CLI command router [`src/aibar/aibar/cli.py`]
    - `show(...)`: usage fetch/report route with per-provider TTL cache [`src/aibar/aibar/cli.py`]
      ...
      - `_fetch_claude_dual(...)`: single-API-call dual-window fetch for Claude with rate-limit cooldown and cross-window raw re-parse fallback on both the cooldown pre-check path and the post-fetch 429 path [`src/aibar/aibar/cli.py`]
        ...
        - `ClaudeOAuthProvider._parse_response(...)`: cross-window raw re-parse for windows without disk cache (derives missing window metrics from sibling last-good raw payload; applied on both cooldown pre-check and post-fetch 429 paths) [`src/aibar/aibar/providers/claude_oauth.py`]
        ...
        - `ClaudeOAuthProvider._parse_response(...)`: cross-window raw re-parse on post-fetch 429 path when last-good is absent but sibling result carries full raw payload [`src/aibar/aibar/providers/claude_oauth.py`]
