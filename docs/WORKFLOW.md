...
- `Internal Call-Trace Tree`
  - `main(...)`: CLI command router [`src/aibar/aibar/cli.py`]
    - `show(...)`: usage fetch/report route with per-provider TTL cache [`src/aibar/aibar/cli.py`]
      ...
      - `_fetch_claude_dual(...)`: single-API-call dual-window fetch for Claude with rate-limit cooldown and cross-window raw re-parse fallback [`src/aibar/aibar/cli.py`]
        ...
        - `ClaudeOAuthProvider._parse_response(...)`: cross-window raw re-parse for windows without disk cache during cooldown (derives missing window metrics from sibling last-good raw payload) [`src/aibar/aibar/providers/claude_oauth.py`]
